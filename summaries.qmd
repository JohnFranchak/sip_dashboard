...
redcap <- board %>% pin_read("redcap_data") %>% 
  ...
  mutate(id = as.character(id), session = as.character(session),
...
aims_long <- redcap %>% rename(aims_total_score = aims_totalscore) %>% pivot_longer(cols = c("aims_supine_total", "aims_prone_total", "aims_sitting_total", "aims_standing_total", "aims_total_score"),
  ...
  mutate(Total = ifelse(Score == "total"))

ggplot(aims_long, aes(x = session, y = Score, color = Scale, group = Scale)) + 
  stat_summary(position = position_dodge(.1)) + 
  stat_summary(geom = "line", position = position_dodge(.1)) + 
  ylim(0,58) + ylab("Score") + xlab("Session") +
  scale_x_discrete(breaks = c("1","2","3","4"), labels = c("7 mo","9 mo","11 mo","13 mo")) + 
  theme(legend.position = "bottom")