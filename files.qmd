...
# VIDEOS
ds_files$GOPRO <- map2_int(ids, sessions, ~ file.exists(str_glue("{study_dir}data/{.x}/{.y}/video_converted/gopro_{.x}_{.y}_pt1.mp4")) | file.exists(str_glue("{study_dir}data/{.x}/{.y}/video_converted/gopro_{.x}_{.y}.mp4")))
...
ds_files$MOT4 <- map2_int(ids, sessions, ~ file.exists(str_glue("{study_dir}data_processed/imu/{.x}_{.y}/mot_features_infant_4s.csv")))
ds_files$IMU4 <- map2_int(ids, sessions, ~ file.exists(str_glue("{study_dir}data_processed/imu/{.x}_{.y}/infant_position_predictions_4s.csv")))
ds_files$Sleep <- map2_int(ids, sessions, ~ file.exists(str_glue("{study_dir}data_processed/lena_sleep_tcds/{.x}_{.y}.csv")))

# CG Processed

ds_files$CGMOT4 <- map2_int(ids, sessions, ~ file.exists(str_glue("{study_dir}data_processed/imu/{.x}_{.y}/mot_features_cg_4s.csv")))
ds_files$CGPOS4 <- map2_int(ids, sessions, ~ file.exists(str_glue("{study_dir}data_processed/imu/{.x}_{.y}/cg_position_predictions_4s.csv")))

```

| Last updated: `r round(now())`
| Showing files for `r nrow(ds_files)` sessions
...
reactable( 
          ...
          columnGroups = list(colGroup("Visit", columns = c("ID", "Session"), sticky = "left"),
                              ...
                              colGroup("LENA", columns = c("ITS","WAV","Sleep")),
                              colGroup("Infant Processing", columns = c("MOT4","IMU4")),
          ...
          CGMOT4 = colDef(maxWidth = 80),
          CGPOS4 = colDef(maxWidth = 80)
          ))
