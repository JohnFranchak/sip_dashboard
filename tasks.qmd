...
ds_raw <- redcap_read_oneshot(redcap_uri = uri, token = api_token, events = events)$data 
...
ds <- ds_raw %>% filter(inclusion == 1 | finished_aims == 1) %>% 
  mutate(session = str_remove(str_remove(redcap_event_name, "visit_"),"_arm_1"),
         session = factor(session, levels = c("1","2","3","4"))) %>% 
  mutate(across(all_of(c("lena_its","imu_sync","imu_sync_cg","wear_time_correction","video_composite")), ~ ifelse(session == 4, -1, .x)),
         server_upload = ifelse(session == 4, upload, server_upload),
         databrary_upload = ifelse(session == 4, upload_2, databrary_upload)) %>% 
  rename(Session = session, ID = study_id) %>% 
  select(ID, Session, surveys_complete, server_upload, databrary_upload, lena_its, imu_sync, imu_sync_cg, video_composite, wear_time_correction, aims_complete, aims_reliability_complete) %>% 
...
tdate_events <- c("intro_call_arm_1", "schedule_2_arm_1", "schedule_3_arm_1","visit_4_arm_1")
ds_tdate <- redcap_read_oneshot(redcap_uri = uri, token = api_token, events = tdate_events, fields = c("study_id", "visit_date","zoom_call_date"))$data %>% 
  mutate(session = factor(redcap_event_name, levels = tdate_events, labels = c("1","2","3","4")),
         Date = as_date(ifelse(session == "4", zoom_call_date, visit_date))) %>% 
        rename(Session = session, ID = study_id) %>% select(-redcap_event_name, -visit_date, -zoom_call_date)

ds <- ds %>% left_join(ds_tdate)

```

| Last updated: `r round(now())`
| Showing files for `r nrow(ds)` sessions
...
ds %>% rename(
  ...
  Wear = wear_time_correction,
  AIMS = aims_complete,
  `AIMS Rel` = aims_reliability_complete) %>% 
  relocate(Date, .after = Session) %>% 
reactable( 
          filterable = TRUE, 
          compact = TRUE,
          striped = TRUE,
          ...
          Date = colDef(format = colFormat(date = TRUE), maxWidth = 95, align = "right"),
          ...
          Wear = colDef(maxWidth = 90, cell = function(value) {if (value == 1) "\u2714\ufe0f" else if (value == 0) "\u274c" else "\u2716"}),
