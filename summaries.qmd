...
ds_sum <- ds_sum %>% complete(nesting(id, session), restraint, fill = list(restraint_n = 0, total_samples = 1))
ds_sum$restraint_prop = ds_sum$restraint_n/ds_sum$total_samples*100
ds_sum$session = as.numeric(ds_sum$session)

ds_sum %>% group_by(restraint, session) %>% 
