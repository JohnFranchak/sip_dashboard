...
tdate_events <- c("intro_call_arm_1", "schedule_2_arm_1", "schedule_3_arm_1")
ds_tdate <- redcap_read_oneshot(redcap_uri = uri, token = api_token, events = tdate_events, fields = c("study_id", "visit_date"))$data %>% 
  mutate(session = str_replace(str_remove(str_remove(redcap_event_name, "schedule_"),"_arm_1"), "intro_call", "1"),
         session = factor(session, levels = c("1","2","3"))) %>% 
          rename(Session = session, ID = study_id) %>% select(-redcap_event_name)

ds <- ds %>% left_join(ds_tdate) %>% rename(Date = visit_date)
